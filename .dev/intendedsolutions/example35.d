...
int findspilt(R)(R r){
	foreach(i,e;r.slide(2).enumerate){
		if(e[0]>e[1]){
			return cast(int)i+1;
		}
	}
...
bool verify(R)(R r){
	...
	r=copy;
	bool pass=r.take(spilt).isSorted;
	r=copy;
	pass&=r.drop(spilt).isSorted;
	if(pass){
		testspassed++;
	}
	return pass;
...
unittest{
	foreach(i;0..100){
		assert((randomsortedarray(0,100)~randomsortedarray(0,100)).verify);
	}
	assert("abcabc".map!(a=>a).array/*ugly hacks cause strings are so misbehaved*/.verify);
